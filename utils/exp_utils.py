...
def create_logger(cfg, cfg_name, phase='train'):
    ...
    mv_data_name = cfg.dataset.mv_data_name
    sv_data_name = cfg.dataset.sv_data_name
    dataset_name = mv_data_name if not cfg.dataset.sv_data_use else mv_data_name + '_' + sv_data_name + '-sv'
...
def to_easydict_recursively(d):
    if isinstance(d, dict):
        # Use a dictionary comprehension to convert each value in the dictionary
        # This ensures that nested dictionaries are also converted to EasyDict
        return edict({k: to_easydict_recursively(v) for k, v in d.items()})
    else:
